...
* Added `gnevar` database
  ...
  * Added variables for membership conditions and membership procedures using
  `manypkgs::code_memberships()` for trade agreements listed 
  in `agreements` database
* Added `texts` database with trade agreements texts
  * Closed #9 by incorporating TOTA database to the `AGR_TXT` dataset
  in the `texts` database.
